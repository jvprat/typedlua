...
local tlchecker = setmetatable({}, { __index = tlvisitor })
...
local function check_var (env, var, exp)
  local tag = var.tag
  if tag == "Id" then
    local name = var[1]
  ...
  elseif tag == "Index" then
    ...
    tlchecker:Expression(exp1, env)
    tlchecker:Expression(exp2, env)
...
function tlchecker:BinaryOp (exp, env)
  local op = exp[1]
...
function tlchecker:Block (block, env)
  ...
  for _, v in ipairs(block) do
    r, _, didgoto = self:Statement(v, env)
    env.self = bkp
...
function tlchecker:Break (stm, env)
  return false
end
...
function tlchecker:Call (exp, env)
  local exp1 = exp[1]
  local explist = {}
  for i = 2, #exp do
    explist[i - 1] = exp[i]
  end
  self:Expression(exp1, env)
  self:ExpList(explist, env)
...
function tlchecker:Dots (exp, env)
  set_type(exp, tltype.Vararg(tlst.get_vararg(env)))
end

function tlchecker:ExpList (explist, env, lselfs)
  lselfs = lselfs or {}
  for k, v in ipairs(explist) do
    self:Expression(v, env, lselfs[k])
  end
end

function tlchecker:False (exp, env)
  set_type(exp, False)
end

function tlchecker:Forin (stm, env)
  local idlist, explist, block = stm[1], stm[2], stm[3]
  tlst.begin_scope(env)
  self:ExpList(explist, env)
  ...
  local r, _, didgoto = self:Block(block, env)
  check_unused_locals(env)
  tlst.end_scope(env)
  return r, _, didgoto
end
...
function tlchecker:Fornum (stm, env)
...
function tlchecker:Function (exp, env, tself)
  local oself = env.self
  env.self = tself
  ...
  local r = tlchecker:Block(block, env)
...
function tlchecker:Goto (stm, env)
  return false, nil, true
end

function tlchecker:Id (exp, env)
  local name = exp[1]
  local l = tlst.get_local(env, name)
  local t = get_type(l)
...
function tlchecker:If (stm, env)
  ...
  for i = 1, #stm, 2 do
    ...
    local r, isret = self:Block(block, env)
    table.insert(rl, r)
...
function tlchecker:Index (exp, env)
...
function tlchecker:Interface (stm, env)
  local name, t, is_local = stm[1], stm[2], stm.is_local
...
function tlchecker:Invoke (exp, env)
  local exp1, exp2 = exp[1], exp[2]
  local explist = {}
  for i = 3, #exp do
    explist[i - 2] = exp[i]
...
function tlchecker:Label (stm, env)
  return false
end
...
function tlchecker:Local (stm, env)
  local idlist, explist = stm[1], stm[2]
  self:ExpList(explist, env)
...
function tlchecker:Localrec (stm, env)
  local id, exp = stm[1][1], stm[2][1]
  local idlist, ret_type, block = exp[1], replace_names(env, exp[2], exp.pos), exp[3]
  local infer_return = false
  if not block then
    block = ret_type
...
function tlchecker:Nil (exp, env)
  set_type(exp, Nil)
end

function tlchecker:Number (exp, env)
  set_type(exp, tltype.Literal(exp[1]))
end

function tlchecker:Paren (exp, env)
  local exp1 = exp[1]
  self:Expression(exp1, env)
  local t1 = get_type(exp1)
...
function tlchecker:Repeat (stm, env)
  local stm1, exp1 = stm[1], stm[2]
  local r, _, didgoto = self:Block(stm1, env)
  self:Expression(exp1, env)
  return r, _, didgoto
end

function tlchecker:Return (stm, env)
  self:ExpList(stm, env)
  local t = explist2typelist(stm)
  tlst.set_return_type(env, tltype.general(t))
  return true
end

function tlchecker:Set (stm, env)
  local varlist, explist = stm[1], stm[2]
  local lselfs = {}
  ...
  self:ExpList(explist, env, lselfs)
  local l = {}
  for k, v in ipairs(varlist) do
...
function tlchecker:String (exp, env)
  set_type(exp, tltype.Literal(exp[1]))
end

function tlchecker:Table (exp, env)
  local l = {}
  local i = 1
  local len = #exp
...
function tlchecker:True (exp, env)
  set_type(exp, True)
end
...
function tlchecker:UnaryOp (exp, env)
  local op = exp[1]
  if op == "not" then
    check_not(env, exp)
  elseif op == "bnot" then
...
function tlchecker:While (stm, env)
